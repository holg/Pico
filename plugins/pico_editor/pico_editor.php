...
*/';
    ...
    public function get_file()
    {
		...
		if(substr($file_url, -1) == '/') {
            $dir = substr($file_url,1, strlen($file_url)-1);
            $file = 'index';
        }else {
            $dir = dirname($file_url).'/';
            $file = basename($file_url);
        }
        $fileabspath = CONTENT_DIR . $dir .$file . CONTENT_EXT;
        if($dir == '/') {
            $dir  = '';
        }
        if($file == ""){
        ...
        return array($file, $file_url, $dir, $fileabspath);
    }
	
	private function do_open()
	{
		list($file, $file_url, $dir, $fileabspath) = $this->get_file();
        if(!$file) die('Error 1: Invalid file');

        if(file_exists($fileabspath)) die(file_get_contents($fileabspath));
		else die('Error 2: Invalid file:'.$fileabspath);
	}
	...
	private function do_save()
	{
		...
		file_put_contents($fileabspath, $content);
		die($content);
	}
	...
	private function do_delete()
	{
		...
		if(file_exists($fileabspath)) die(unlink($fileabspath));
	}
